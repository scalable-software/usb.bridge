// MicroView font data, machine-converted from the C headers in oled/src/util
// (GPL-3.0, from the MicroView library by GeekAmmo). Each data byte is a
// vertical 8-pixel slice, least significant bit at the top. Glyphs sit side
// by side in a bitmap mapWidth pixels wide; fonts taller than 8 pixels
// continue in full-width rows of height/8 page slices.

export interface OledFont {
  width: number;
  height: number;
  startChar: number;
  totalChars: number;
  mapWidth: number;
  data: Uint8Array;
}

export const LARGE_NUMBERS: OledFont = {
  width: 12,
  height: 48,
  startChar: 48,
  totalChars: 11,
  mapWidth: 132,
  data: Uint8Array.from([
  ,
  ]),
};

export const FONT_5X7: OledFont = {
  width: 5,
  height: 8,
  startChar: 0,
  totalChars: 255,
  mapWidth: 1275,
  data: Uint8Array.from([
  ,
  ]),
};
